...
public interface SaraRelationRepository extends CrudRepository<SaraRelation, String>{
    ...
    //TODO usage of incorrect repository to retrieve object from join table, it happens because join table is not a ->
    //TODO -> just a join table but also occupies some data, in such case new Entity should be created.
	...
	@Query(nativeQuery=true, value="select f.id as fundId, f.name as fundName, c.type as countryCode, c.label as countryName, r.rad,g.name as regionName, sum(r.asset_Value), count(distinct e.id) from country c, region g, sara_legal_fund f, sara_entity e, sara_relation r where c.region_id = g.id and r.lf_id = f.id and r.bp_id = e.id and e.residence_code = c.type group by f.id,c.type, r.rad,g.id")
